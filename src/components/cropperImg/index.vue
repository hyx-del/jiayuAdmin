 ...
 * @LastEditors: zx
 * @LastEditTime: 2021-10-29 10:06:12
 * @Descripttion:  图片剪切
-->
	...
	<div>
		<div class="lt_upload_img_list">
			<div class="lt_cropper_input">
				<div class="upload_img_content" v-for="(imgItem, index) in fileList" :key="imgItem.name">
				...
				<input v-if="limit == fileList.length" title="" ref="inputer" type="file" name="file" accept="jpg" @change="handleImgChange" class="lt_cropper_input_file" />
			</div>

			<div v-if="limit != fileList.length" class="lt_cropper_input lt_cropper_upload_item el-upload--picture-card">
				<i class="el-icon-plus avatar-uploader-icon"></i>
				<input ref="inputer" title="" type="file" name="file" accept="jpg" @change="handleImgChange" class="lt_cropper_input_file" />
			</div>
		</div>
		<el-dialog width="700" title="图片剪切" :visible.sync="cropperImgVisible" append-to-body :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
...
export default {
	...
	props: {
		...
		size: {
			type: Number, //	size: this.size, // 图片默认大小限制  400kb
			default: 500,
		},
		maxImgSize: {
			type: Number,
	...
	watch: {
		ratio(newValue, oldVal) {
			// console.log('newValue, oldVal', newValue, oldVal);
			this.$forceUpdate();
			// console.log(
			...
			// );
			this.option.fixedNumber = [1, newValue];
			this.option.autoCropHeight = this.width * newValue;
		},
	...
	computed: {
		...
		getStoreQNToken() {
			// console.log('==crop==', store.getters.upLoadToken);
		...
		handleImgChange(e) {
			...
			reader.onload = e => {
				...
				if (typeof e.target.result === 'object') {
					this.option.cropperImgObj = window.URL.createObjectURL(new Blob([e.target.result]));
				} else {
