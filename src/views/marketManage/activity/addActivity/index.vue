 ...
 * @LastEditTime: 2021-07-15 11:11:06
 * @Descripttion: 
-->
<template>
	<div class="addActivity">
		...
		>
			<el-row>
				...
				<el-col :span="12">
					<div>
						<h2>规则说明：</h2>
						<p>1、活动开始时间＜活动结束时间。</p>
						<p>2、限时抢购活动不允许有时间的交叉。</p>
						<!-- <p>
							3、抢购人数倍数(虚拟):前端展示的购买人数=实际购买人数*X倍，需要填写倍数数值。
						</p> -->
						<p>3、选择商品规则：不能选择参与其他活动的商品（团购）。</p>
					</div>
				</el-col>
			</el-row>
					...
					<div v-for="(itemTemp, index) in ruleForm.items" :key="index">
						...
						<div>
							<el-button
								...
								@click="openFillDialog(itemTemp, index)"
							>
								批量填充
							</el-button>
						</div>
						...
						>
							...
							<el-table-column
								...
								label="规格名称"
							></el-table-column>
							<el-table-column align="center" prop="skuRushPrice" label="抢购价">
								<template slot-scope="scope">
									<span>
										{{
											scope.row.skuRushPrice
												? `${moneyFormat(scope.row.skuRushPrice)}元`
												: ''
										}}
									</span>
								</template>
							</el-table-column>
							<el-table-column align="center" prop="skuOriginalPrice" label="划线价">
								<template slot-scope="scope">
									<span>
										{{ `${moneyFormat(scope.row.skuOriginalPrice)}` }}元
									</span>
								</template>
							</el-table-column>
							<el-table-column align="center" prop="skuPrice" label="未划线价">
								<template slot-scope="scope">
									<span>{{ `${moneyFormat(scope.row.skuPrice)}` }}元</span>
								</template>
							</el-table-column>
							<el-table-column
								align="center"
			...
			<div v-if="!ruleForm.fareConfig.freeShip">
				<el-form-item label="默认运费：" prop="fareConfig.fare">
					<el-input
						placeholder
						v-model.number="ruleForm.fareConfig.fare"
						maxlength="3"
						size="small"
					>
						<template slot="append">
				...
				<div class="parallel">
					...
					<el-form-item label-width="90px" label="运费为：" prop="fareConfig.farePrice">
						<el-input
							placeholder
							v-model.number="ruleForm.fareConfig.farePrice"
							maxlength="3"
							size="small"
						>
							<template slot="append">
		...
		>
			...
			>
				...
				<el-form-item label="每人限购数量" style="margin-bottom:0">
					<div>
						<el-switch
							v-model="checked"
							active-color="#13ce66"
				...
				<el-form-item prop="limitNum" v-if="checked">
					<el-input
		...
		>
			...
			>
				<el-form-item label="抢购价" prop="skuRushPrice">
					<el-input
						size="small"
						v-model.trim="fillSpuForm.skuRushPrice"
						maxlength="10"
					></el-input>
				</el-form-item>
				<el-form-item label="活动库存" prop="inventoryActNum">
				...
				<el-form-item label="每人限购数量" style="margin-bottom:0">
					<div>
						<el-switch
							...
							inactive-color="#ff4949"
							style="margin-bottom:10px"
							@change="fillFullChange"
						></el-switch>
					</div>
				</el-form-item>
				<el-form-item prop="limitNum" v-if="fillChecked">
					<el-input
						placeholder="请输入每人限购数量"
						v-model.number="fillSpuForm.limitNum"
						size="small"
						maxlength="3"
						@change="fillItemChange"
					>
						<template slot="append">
							/个
						</template>
...
export default {
	...
	methods: {
		fillFullChange(status) {
			// console.log(status);
			if (status) {
				this.fillSpuForm.limitNum = '';
			} else {
				this.fillSpuForm.limitNum = -1;
			}
		},
		fillItemChange(status) {
			// console.log(status);
			if (status) {
				this.SkuForm.limitNum = '';
			} else {
		...
		onClickSkuBtu() {
			...
			if (this.checkList.length < 1) {
				...
				this.ruleForm.items.forEach((spuItem, index) => {
					//判断已经勾选的spu 中是否有相同的spu 信息
					//  ;
					if (this.spuInfo.id == spuItem.itemId) {
						sameSpuItemInfo.index = index;
						sameSpuItemInfo.spuItem = spuItem;
						spuItem.itemSkuList.forEach(skuItem => {
							if (_that.checkList.includes(skuItem.itemSkuId)) {
								sameSpuItemInfo.skuArray.push(skuItem.itemSkuId);
							}
						});
					}
				});
						...
						this.skuData.forEach(skuItem => {
							...
							if (element === skuItem.id) {
								...
								if (sameSpuItemInfo.skuArray.includes(skuItem.id)) {
									...
									const _tempSkuItem = {
										itemSkuImage: skuItem.img, // 关联商品 sku 图片
										itemSkuName: skuItem.name, // 关联商品 sku 规则
										skuOriginalPrice: skuItem.originPrice, //  sku 原价
										skuPrice: skuItem.price, // sku 现价
										itemSkuId: skuItem.id, // 关联商品 skuId
										properties: skuItem.properties, // SKU的属性 【颜色:绿色;内存:64G;版本:公开版】
										stock: skuItem.stock, // SKU 库存
										skuRushPrice: null, // sku抢购价格
										limitNum: null, // 单人限购数量，若不限制为-1
										inventoryActNum: null, // 活动库存
										itemId: skuItem.itemId, // 商品名称
									};
									this.ruleForm.items[sameSpuItemInfo.index].itemSkuList.push(
										_tempSkuItem
									);
								}
							}
		...
		setFillValToSKU(formName) {
			...
			this.$refs[formName].validate(valid => {
				if (valid) {
					// if (this.fillSpuForm.skuPrice < this.fillSpuForm.skuRushPrice) {
					// 	this.$message.error('抢购价格不能大于未划线价格');
					// 	return false;
					// }
					// if (this.fillSpuForm.stock < this.fillSpuForm.inventoryActNum) {
					// 	this.$message.error('活动库存不能大于SKU商品库存');
					// 	return false;
					// }
					this.fillSpuItem.itemSkuList.forEach(element => {
						// console.log(element);
						element.inventoryActNum = this.fillSpuForm.inventoryActNum;
						element.limitNum = this.fillSpuForm.limitNum;
						element.skuRushPrice = this.fillSpuForm.skuRushPrice;
					});
