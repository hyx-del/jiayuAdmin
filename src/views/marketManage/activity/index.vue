 ...
 * @LastEditTime: 2021-05-17 17:11:09
 * @Descripttion:
-->
<template>
	<div class="activity">
		<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline">
			...
			<!-- <el-form-item label="" prop="termination">
				<el-select
					v-model.trim="formInline.termination"
					placeholder="活动状态"
					size="small"
				>
					<el-option label="全部" value="" />
					<el-option label="已终止" :value="true" />
					<el-option label="未终止" :value="false" />
				</el-select>
			</el-form-item> -->
...
import { getActivityList, banState, removeActivity, termination } from '@/api/activity';
					...
					actives: [
						{
							name: val => {
								if (!val.termination) {
									if (
										val.rushBeginTime < new Date().getTime() &&
										val.rushEndTime > new Date().getTime()
									) {
										return '终止活动';
									}
								}
							...
							method: val => {
								this.$confirm('确定要终止这个活动吗?', '提示', {
									...
									termination({ id: val.id }).then(res => {
										if (res.success) {
											this.$message({
												type: 'success',
												message: '操作成功！！！',
						...
						},
						{
							...
							method: val => {
								this.$confirm('确定要移除这个活动吗?', '提示', {
									type: 'warning',
								}).then(() => {
									removeActivity({ id: val.id }).then(res => {
										if (res.success) {
											this.$message({
												type: 'success',
												message: '移除成功！！！',
