  ...
  methods: {
    ...
    formButton(val){
      if (val.name=='confirm'&&val.valid) {
        let data=val.value
        if (this.edit) {
          jananColumnUpdatePut(data).then(res=>{
          ...
          // 新加
          jananColumnSavePost(data).then(res=>{
            this.dialogFormVisible=false
            this.$refs.TableList.update({})
          }).finally(()=>{
            this.dialogFormVisible=false
