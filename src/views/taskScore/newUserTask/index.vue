 ...
 * @LastEditTime: 2021-06-26 09:23:47
 * @Descripttion: 新手任务
-->
<template>
	<div class="signConfigComponent">
		...
		>
			<Table
				...
				:params="logQueryInfo"
				:loadData="getNewUserTaskLogListFun"
				:column="logColumn"
...
import { getNewUserTaskRule, editNewUserTaskRule, getNewUserTaskLogList } from '@/api/taskScore';
...
export default {
	...
	computed: {
		getNewUserTaskLogListFun() {
			return getNewUserTaskLogList;
		},
	},
	mounted() {
		this.initData();
		...
		handleOK(formName) {
			this.$refs[formName].validate(valid => {
				if (valid) {
					...
					// 编辑
					editNewUserTaskRule(_tempObj).then(res => {
						if (res.code === 200) {
							this.handleClose();
							this.getTableList();
