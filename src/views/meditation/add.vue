...
import { mediationColumnList, mediationDetail, mediationSavePost, mediationUpdatePost } from "@/api/meditation";
...
export default {
   ...
   mounted() {
      if (this.$route.query.id) {
         this.edit = true;
         mediationDetail({ id: this.$route.query.id }).then((res) => {
            console.log(res);
            let data = res.data;
            let anchorPoint = data.anchorPoint;
            data.anchorPoint = Math.floor(anchorPoint / 60) + ":" + parseInt(anchorPoint - Math.floor(anchorPoint / 60) * 60);
            this.formObj = data;
         });
      }
   },
   ...
   methods: {
      formButton(val) {
         if (val.name == "confirm" && val.valid) {
            ...
            data.type = +data.type;
            if (data.anchorPoint) {
               let anchorPoint = data.anchorPoint.replace(/\：/, ":");
               let reg = /^\d+[\:\：]\d{1,2}$/;
               if (anchorPoint == "" || reg.test(anchorPoint)) {
                  let one = anchorPoint.split(":");
                  let two = parseInt(one[1]);
                  data.anchorPoint = parseInt(one[0]) * 60 + two;
               } else {
                  this.$message.error("锚点(分秒)格式 00:00");
                  return;
               }
            }
            data.meditationColumnVOS = data.meditationColumnVOS.map((item) => {
