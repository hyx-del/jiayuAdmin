  ...
  <div class="commonPage">
    <div class="myDetail">
      ...
      <div class="detail_left">
        <div class="detail_title">冥想基本信息</div>
        <div class="formShowH">
...
export default {
  ...
  computed: {
    ...
    courseList() {
      return mediationUserPracticeList;
    },
  },
  created () {
    if (this.$route.query.id) {
      ...
      mediationDetail({id:this.$route.query.id}).then(res=>{
        let mid={}
        res.data.meditationColumnVOS.forEach(item=>{
          item.code='meditationColumnVOS'+item.id
          item.label=item.name+'顺序'
          this.list.splice(1,0,item)
          mid['meditationColumnVOS'+item.id]=item.sort
        })
  ...
  mounted () {
    // this.$refs.Table.update({ courseId: this.id});
  },
  methods: {
    backList(){
      this.$router.push({
