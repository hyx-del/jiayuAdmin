   ...
   <div class="commonPage">
      <div class="myDetail">
         ...
         <div class="detail_left">
            <div class="detail_title">资讯详情</div>
            <div class="formShowH">
               ...
               <div class="formShowH_mid">
                  <div class="formShowH_tips">
                     {{ formObj.createTime || formObj.startTime }}
                  </div>
                  <div class="formShowH_tips">点赞：{{ formObj.praise || 0 }}</div>
...
import { putWikiCoach, wikiColumnList, wikiList } from "@/api/wiki";
import { getNews, newsGetCoach, newsGetCourse, newsPutCoach, newsPutCourse } from "@/api/news";
...
export default {
   ...
   created() {
      if (this.$route.query.id) {
         ...
         getNews({ id: this.id }).then((res) => {
            this.formObj = res.data;
         });
         newsGetCourse({ id: this.id }).then((res) => {
            this.courseData = res.data;
         });
         newsGetCoach({ id: this.id }).then((res) => {
   ...
   methods: {
      ...
      courseSave() {
         let arr = this.courseData.map((v) => {
            ...
            return obj = { courseId: v.id, sort: sort };
         });
         // courses
         newsPutCourse({ id: this.id, courses: arr }).then((res) => {
      ...
      coachSave() {
         ...
         newsPutCoach({ id: this.id, coaches: arr }).then((res) => {
            this.$message.success("操作成功");
         });
      },
      // 教练选择编辑
