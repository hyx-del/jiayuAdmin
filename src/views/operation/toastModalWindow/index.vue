 ...
 * @LastEditTime: 2021-11-16 10:17:23
 * @Descripttion: 首页弹窗管理
-->
<template>
		...
		>
			<div class="popBody">
				...
				<el-form ref="popBeforeForm" :inline="false" :model="ruleForm" labelWidth="8em">
					<el-form-item label="关联类型：" prop="tabId">
						<el-radio-group @change="changeRadio" size="small" v-model="ruleForm.tabId">
							<el-radio-button
					...
					<el-form-item v-if="ruleForm.tabId != 9" label="选择关联数据：">
						<el-button type="primary" size="small" @click="targe">关联数据</el-button>
					</el-form-item>
					<el-form-item size="small" v-if="ruleForm.tabId == 9" label="h5网址：">
						<el-input style="width: 360px; " v-model="ruleForm.h5"></el-input>
					</el-form-item>
				</el-form>
				<simpleTable
					v-if="simpleObj.column.length && ruleForm.tabId != 9"
					:style="{ height: '180px' }"
	...
	data() {
		return {
			...
			column: [
				...
				{
					prop: 'relevanceValueName',
					label: '关联目标',
					formatter: (row, column) => {
						if (row.tabId == 9) {
							return row.relevanceValue;
						} else {
							return row.relevanceValueName;
						}
					},
				},
				{
					...
					actives: [
						{
							...
							method: val => {
								...
								popup({ id: val.id }).then(res => {
									let data = res.data;
									if (tempObj.tabId) {
										tempObj.tabId = tempObj.tabId * 1;
									}
									if (data.tabId == 9) {
										tempObj.h5 = data.relevanceValue;
									} else {
										this.simpleObj = this.getColumn(this.simpleObj);
										this.simpleObj.chose = [data.popupResourceVO];
									}
	...
	methods: {
		...
		formButton(val) {
			if (val.name == 'confirm' && val.valid) {
				...
				// if (data.tabId==9) {
				//   data.relevanceValue=this.ruleForm.h5
				// }
				data.relevanceValue =
					data.tabId != 9 ? this.simpleObj.chose[0].id || '' : this.ruleForm.h5;
				if (data.tabId == 9 && !urlReg.test(data.relevanceValue)) {
