 ...
 * @LastEditTime: 2021-02-05 14:44:57
		...
		<el-card :body-style="{ padding: '20px' }">
			...
			<el-row>
				<el-col>
					...
					<el-button @click="addGoodsBtu" type="primary" size="small">
						+添加商品
					</el-button>
				</el-col>
			</el-row>
		...
		>
			<el-form ref="goodSearchForm" :model="goodsParmas" :inline="true">
				...
				<el-form-item prop="actType">
					<el-select
						v-model.trim="goodsParmas.actType"
						size="small"
						placeholder="选择活动类型"
					>
			...
			<el-row :gutter="12">
				...
				<el-col :span="12" style="height: 500px;overflow-y: auto;">
					<el-table :data="chooseGoods" ref="chooseForm" :border="true">
						<el-table-column
							:prop="item.prop"
							:label="item.label"
						...
						>
							...
							<template v-slot="scope" v-else-if="item.prop == 'actives'">
								<el-button @click="delChoose(scope)" type="text" size="small">
									删除
								</el-button>
							</template>
						</el-table-column>
...
export default {
	...
	methods: {
		addGoodsBtu() {
			if (this.type === 'SEARCH') {
				getGoodsRecommends({ type: 'SEARCH' }).then(res => {
					console.log(res);
					if (res.data.list.length > 8) {
						this.addVisible = false;
						Message.error('最多仅允许创建8个热搜词条！！！');
					} else {
