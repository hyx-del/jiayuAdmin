		...
		>
			<el-form-item label="全场商品是否包邮：" prop="freeShip" label-width="150px">
				<el-radio-group
					@change="changeRadio"
					:disabled="disabled"
					v-model.trim="ruleForm.freeShip"
				>
			...
			<div v-if="!ruleForm.freeShip">
				...
				<div style="padding:0 0 20px 150px">
					<el-checkbox
						:disabled="disabled"
						v-model="preferentialCheckVal"
						label="运费优惠"
					></el-checkbox>
				</div>
				<div class="parallel" v-if="preferentialCheckVal">
					<el-form-item label="商品合计价格满：" prop="priceAchieve" label-width="150px">
						<el-input
							...
							size="small"
							:disabled="disabled || !preferentialCheckVal"
						>
							<template slot="append">
								/元
							</template>
					...
					<el-form-item label-width="90px" label="运费为：" prop="farePrice">
						<el-input
							placeholder
							v-model.number="ruleForm.farePrice"
							maxlength="3"
							size="small"
							disabled
						>
			...
			<el-form-item style="margin-top:20px">
				<el-button type="primary" v-if="!disabled" @click="submitForm('ruleForm')">
					保存
				</el-button>
				<el-button type="primary" v-else @click="disabled = false">
					修改运费规则
				</el-button>
			</el-form-item>
		</el-form>
...
export default {
	data() {
		...
		return {
			preferentialCheckVal: false, // 商品价格优惠值
			ruleForm: {
				fare: null,
				farePrice: 0,
				freeShip: false,
		...
		getDetail() {
			getFreight().then(res => {
				if (res.code === 200) {
					...
					if (!res.data.farePrice) {
						this.ruleForm.farePrice = 0;
					}
					const _farePrice = res.data.farePrice + '';
					const _priceAchieve = res.data.priceAchieve + '';
					if (_farePrice === 'null' || _priceAchieve === 'null') {
						this.preferentialCheckVal = false;
					} else {
						this.preferentialCheckVal = true;
					}
				} else {
		...
		submitForm(formName) {
			this.$refs[formName].validate(valid => {
				if (valid) {
					...
					if (!this.preferentialCheckVal) {
						_par.farePrice = null;
						_par.priceAchieve = null;
					}

					saveFreight(_par).then(res => {
						if (res.code === 200) {
