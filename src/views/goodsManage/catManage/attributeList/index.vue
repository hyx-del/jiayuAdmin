...
export default {
	...
	methods: {
		...
		// 获取上级类目列表
		getCatInfo() {
			getAllCat({ parentId: this.parentId, size: 100 }).then(
				res => {
					if (res.success) {
		...
		submitForm(formName) {
			this.$refs[formName].validate(valid => {
				if (valid) {
					...
					_ruleForm.catName = _ruleForm.catName.replace(/\s+/g, '');
					this.options.forEach(element => {
						if (element.name === _ruleForm.catName) {
							_ruleForm.catId = element.id;
						}
					});
					_ruleForm.value = this.inputValue.toString();
					if (this.dialogTitle === '添加SKU') {
						if (this.inputValue.length > 0) {
							createCatSkus(_ruleForm).then(
								res => {
									if (res.success) {
					...
					} else if (_ruleForm.value) {
						editCatSkus(_ruleForm).then(res => {
							if (res.success) {
