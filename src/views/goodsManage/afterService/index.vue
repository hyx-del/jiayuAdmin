 ...
 * @LastEditTime: 2020-10-27 10:57:30
 * @LastEditors: wei.chen
 * @Description: 
-->
<template>
	<el-tabs v-model="activeName" type="border-card">
		<el-tab-pane name="fare" label="运费规则">
			<freightRules />
		</el-tab-pane>
		<el-tab-pane name="service" label="支持服务">
			<div>
				<el-row>
					<el-col :span="12">
						<el-button
...
export default {
	...
	mounted() {
		if (this.$route.query.from === 'addSku') {
			this.activeName = 'service';
		} else {
			this.activeName = 'fare';
		}
	},
	computed: {
		getService() {
	...
	methods: {
		...
		create() {
			this.$refs.adminForm.validate(valid => {
				if (valid) {
					createService({ ...this.adminForm, parentId: this.parentId }).then(res => {
						if (res.success) {
